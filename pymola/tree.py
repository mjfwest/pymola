...
def flatten_class(root: ast.Collection, orig_class: ast.Class, instance_name: str,
    ...
    # A flag whether we need to check symbol types on whether they are a class
    # or not. If we load a class from cache, we have already flattened it (and
...
def flatten_component_refs(
    ...
    if not inplace:
        expression = copy_fast(expression)
...
def pull_functions(root: ast.Collection, expression: ast.Expression, instance_prefix: str) -> set:
    ...
    expression_copy = copy_fast(expression)
