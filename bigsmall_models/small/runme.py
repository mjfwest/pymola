...
db_file = os.path.join(TEST_DIR, 'SmallModel')
try:
    os.remove(db_file)
except:
    pass

for f in glob.glob(os.path.join(TEST_DIR, "SmallModel*.so")):
    os.remove(f)
for f in glob.glob(os.path.join(TEST_DIR, "SmallModel*.dll")):
    os.remove(f)
for f in glob.glob(os.path.join(TEST_DIR, "SmallModel*.dylib")):
    os.remove(f)
...
ref_model = transfer_model(TEST_DIR, 'SmallModel', compiler_options)

# Load from cache:
# cached_model = transfer_model(TEST_DIR, 'SmallModel', compiler_options)
